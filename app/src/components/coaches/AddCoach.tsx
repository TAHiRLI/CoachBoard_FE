...
import { CoachPostDto } from "@/lib/types/coach.types";
import { Save } from "@mui/icons-material";
import { createCoach } from "@/store/slices/coaches.slice";
...
const AddCoach = ({ onSuccess, onCancel }: Props) => {
  ...
  const formik = useFormik<CoachPostDto>({
    initialValues: {
      fullName: "",
      email: "",
      teamId: 0,
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="flex flex-col gap-4">
      <h1 className="text-2xl my-2 text-center font-bold">{t("static.createCoach")}</h1>
