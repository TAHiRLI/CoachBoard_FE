...
import { CreatePlayerBmiRecordDto } from "@/lib/types/playerBmi.types";
import { playerBmiRecordsService } from "@/API/Services/playerBmiRecords.service";

interface AddPlayerBmiRecordProps {
  playerId: string;
  onSuccess?: () => void;
  onCancel?: () => void;
}

const AddPlayerBmiRecord: React.FC<AddPlayerBmiRecordProps> = ({ playerId, onSuccess, onCancel }) => {
  ...
  const formik = useFormik<CreatePlayerBmiRecordDto>({
    initialValues: {
    ...
    onSubmit: async (values) => {
      setSubmitError("");
      try {
        await playerBmiRecordsService.create({
          ...values,
          playerId: Number(playerId),
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <h1 className="text-2xl my-2 text-center font-bold">{t("static.addBmiRecord")}</h1>
...
export default AddPlayerBmiRecord;
